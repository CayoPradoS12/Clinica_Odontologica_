...
-- VIEW 3: Estoque com itens vencidos
...
-- Nenhum acesso via root é utilizado pela aplicação ou pelos perfis abaixo.
-- ================================================================

-- Nível 1: Aplicação principal (backend API) — CRUD completo
...
-- Nível 2: Painel frontend (readonly) — somente leitura nas views
...
-- Nível 3: Perfil Administrador — acesso total ao schema
-- Justificativa: Administrador precisa visualizar e gerenciar todos os dados do sistema
create user if not exists 'adm_clinica'@'localhost' identified by 'Adm2026!';
grant select, insert, update, delete on clinica_odontologica.* to 'adm_clinica'@'localhost';

-- Nível 4: Perfil Dentista — acesso à agenda e dados clínicos dos pacientes
-- Justificativa: Dentista consulta sua agenda e histórico dos pacientes, sem acesso financeiro
create user if not exists 'dentista_clinica'@'localhost' identified by 'Dentista2026!';
grant select on clinica_odontologica.vw_agenda_completa to 'dentista_clinica'@'localhost';
grant select on clinica_odontologica.consulta to 'dentista_clinica'@'localhost';
grant select on clinica_odontologica.cliente to 'dentista_clinica'@'localhost';
grant select on clinica_odontologica.usuarios to 'dentista_clinica'@'localhost';
grant select on clinica_odontologica.procedimento to 'dentista_clinica'@'localhost';
grant select on clinica_odontologica.consulta_procedimento to 'dentista_clinica'@'localhost';

-- Nível 5: Perfil Recepcionista — agenda e estoque, sem acesso financeiro
-- Justificativa: Recepcionista agenda consultas e controla estoque, mas não acessa dados financeiros
create user if not exists 'recepcao_clinica'@'localhost' identified by 'Recepcao2026!';
grant select on clinica_odontologica.vw_agenda_completa to 'recepcao_clinica'@'localhost';
grant select on clinica_odontologica.vw_estoque_vencido to 'recepcao_clinica'@'localhost';
grant select on clinica_odontologica.usuarios to 'recepcao_clinica'@'localhost';
grant select on clinica_odontologica.cliente to 'recepcao_clinica'@'localhost';
grant insert on clinica_odontologica.consulta to 'recepcao_clinica'@'localhost';
